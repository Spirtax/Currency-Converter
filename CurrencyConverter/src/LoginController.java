...
public class LoginController implements Initializable {
    ...
    public void initialize(URL arg0, ResourceBundle arg1) {
        passwordField.textProperty().addListener((observable, oldValue, newValue) -> {
            this.loginPasswordInput = newValue;
            System.out.println(loginPasswordInput);
        });
        
        usernameField.textProperty().addListener((observable, oldValue, newValue) -> {
        	this.loginUserInput = newValue;
        	System.out.println(loginUserInput);
        });
    ...
    private void handleSignIn(ActionEvent event) {
        ...
        if (event.getSource() == loginButton) {
            try {
                // Call the validUser method with the entered username and password
                boolean isValidUser = userlogin.validUser(usernameField.getText(), passwordField.getText());

                if (isValidUser) {
                    // Switch to the main page if login is successful
                    switchToMain(event);
                } else {
